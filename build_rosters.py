...
def extract_course_name(name_col: str, location: str = "") -> str:
    """
    Clean course name from the Name column; always ends with 'Training', e.g.
    '... Level C Training Markham' -> '... Level C Training'
    'First Aid Instructor Recertification Training North York'
    ...
    m = re.search(r"\bTraining\b", after, flags=re.I)
    if m:                                          # 'Training' marks the end
        core = after[:m.start()].strip()
