...
def detect_course_type(name_col: str) -> str:
    ...
    if "in-class" in low or "in class" in low:
        return "in-class"
    return None  # unknown course type -> caller will skip


def classify_template(path: str) -> str:
    """Identify a template by its actual layout (filenames are unreliable)."""
    d = Document(path)
...
def fill_participants(doc, participants):
    ...
    for i, (name, note) in enumerate(participants):
        ...
        if note:
            set_cell(data_rows[i].cells[-1], note)
...
def build_one(rec, template_map, out_dir, sort_alpha=True, include_cancelled=False):
    ...
    if ctype is None:
        return ("skipped", f"unrecognised course type ({extract_course_name(name_col)})",
                None, 0)
